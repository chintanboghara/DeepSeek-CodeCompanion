...
def display_sidebar():
    ...
    Args:
        disabled_flag (bool): Whether LLM-dependent widgets should be disabled.
    ...
    with st.sidebar:
        ...
        clear_history_button = st.button("Clear Chat History") # This button remains active
        st.divider()
...
# --- LLM Engine Initialization with Caching and Error Handling ---
# (This block was modified in the previous step, ensure it's compatible)
# Conditional LLM Engine Initialization & Caching
needs_reinitialization = (
    st.session_state.llm_engine_instance is None or
    selected_model != st.session_state.current_llm_model_name or # This line seems to reference selected_model before it's defined if we move display_sidebar call
...
if needs_reinitialization:
    # Get parameters from a temporary sidebar call if not yet available
    # This is a bit of a workaround due to Streamlit's execution model.
    # Ideally, config is read once.
    # We will call display_sidebar() properly after engine init.
    temp_selected_action, temp_selected_model, temp_temperature, temp_top_k, temp_top_p, _ = display_sidebar(disabled_flag=True) # Temporarily disable to get values

    with st.spinner(f"Initializing AI model: {temp_selected_model}... Please wait."):
        try:
            llm_engine = init_llm_engine(temp_selected_model, temp_temperature, temp_top_k, temp_top_p, OLLAMA_BASE_URL)
            st.session_state.llm_engine_instance = llm_engine
            st.session_state.current_llm_model_name = temp_selected_model
            st.session_state.current_temperature = temp_temperature
            st.session_state.current_top_k = temp_top_k
            st.session_state.current_top_p = temp_top_p
            st.session_state.critical_error_message = None
            logging.info("LLM Engine initialized successfully.")
        except requests.exceptions.ConnectionError as e:
            error_msg = f"Connection Error: Failed to connect to Ollama at {OLLAMA_BASE_URL}. Please ensure Ollama is running and accessible."
            st.session_state.critical_error_message = error_msg
            st.session_state.llm_engine_instance = None
            llm_engine = None
            logging.error(f"Ollama connection error: {e}")
        except Exception as e:
            error_msg = f"Error: Failed to initialize LLM engine. Details: {str(e)}"
            st.session_state.critical_error_message = error_msg
            st.session_state.llm_engine_instance = None
            llm_engine = None
            logging.error(f"LLM engine initialization failed: {e}", exc_info=True)
else:
    if st.session_state.llm_engine_instance:
        llm_engine = st.session_state.llm_engine_instance
    else:
        if not st.session_state.critical_error_message:
            st.session_state.critical_error_message = "LLM Engine not available. Please check logs or try reloading."
        llm_engine = None

# Determine if UI elements dependent on LLM should be disabled
ui_disabled_due_to_error = st.session_state.llm_engine_instance is None

# Display critical error message if any
...
# Now call display_sidebar with the determined disabled state
selected_action, selected_model, temperature, top_k, top_p, clear_history_pressed = display_sidebar(disabled_flag=ui_disabled_due_to_error)
...
# Conditional LLM Engine Initialization & Caching logic moved above display_sidebar
# The llm_engine variable is now defined before this point or is None if init failed.
...
if st.session_state.active_llm_task:
    ...
    if status == "completed":
        ...
        st.toast(f"LLM processing error: {error_details_val}", icon="❌")
...
# User Input Handling
# Chat input is disabled if an LLM task is active OR if the LLM engine itself failed to initialize
chat_input_disabled = (st.session_state.active_llm_task is not None) or ui_disabled_due_to_error
user_query = st.chat_input("Type your coding question here...", disabled=chat_input_disabled, key="user_query_input")

if user_query and user_query.strip() and not chat_input_disabled: # Also ensure not disabled by critical error
