...
- **Improved Error Display**:
    - **Critical Errors**: If the application fails to connect to the Ollama server during startup or model initialization, a prominent error message will be displayed at the top of the page, and LLM-dependent UI elements (like model selection and chat input) will be disabled until the connection is restored.
    - **In-Chat LLM Errors**: If an error occurs while the AI is processing your query, an immediate toast notification will alert you, and the error message will also be displayed directly in the chat history for context.
